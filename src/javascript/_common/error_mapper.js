...
const mapErrorMessage = (error) => {
    ...
    // Check for subcode first, then fall back to code for backward compatibility
    const error_code = error.subcode || error.code;

    // If neither subcode nor code exists, return the backend message or default
    if (!error_code) {
    ...
    // Map error code to localized message with parameter substitution
    switch (error_code) {
        ...
        case 'TradingNotAllowed':
            return localize('Trading is not allowed for this contract type.');
        case 'TradingNotOffered':
            return localize('Trading is not offered for this contract type.');
        case 'TradingTemporarilyUnavailable':
            return localize('Trading is temporarily unavailable.');
        case 'TurnoverLimitExceeded':
            return localize(
                'Purchasing this contract will cause you to exceed your turnover limit of {{param_1}} {{param_2}}.',
                {
                    param_1: params[0],
                    param_2: params[1],
                }
        ...
        case 'UnableToSell':
            return localize('Unable to sell this contract.');
        case 'UnderlyingNotAvailable':
            return localize('This underlying is not available for this contract type.');
        case 'UnknownContractType':
            return localize('Unknown contract type.');
        case 'UnknownError':
        ...
        case 'UnsupportedMarket':
            return localize('This market is not supported.');
        case 'UpdateNotAllowed':
            return localize('Update is not allowed for this contract.');
        case 'ValidationError':
            return localize('Validation error.');
        case 'ZeroAbsoluteBarrier':
            return localize('Absolute barrier cannot be zero.');

        // If error code is not mapped, return the backend message or default
        default:
            return error.message || localize('An error occurred. Please try again later.');
    }
};
